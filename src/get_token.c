...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/03 14:14:15 by sjeuneje          #+#    #+#             */
/*   Updated: 2022/05/03 14:14:15 by sjeuneje         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tok	*ft_consume_token(char **straddr, t_tok *tok)
{
	...
	*straddr = ft_skip_whitespace(*straddr);
	if (!(**straddr))
	{
		tok->type = END;
		return (tok);
...
static t_tok	*ft_consume_redir(char **straddr, t_tok *tok, char *redir_str)
{
	...
	*straddr += ft_strlen(redir_str);
	...
	tok->data.redir->filename = fname_tok.data.str;
...
static size_t	ft_endtoken_index(char *str, int *errnum)
{
	char *const	blanks = " \t\n\r\v\f";
	char *const	ops[10]
		= {">>", ">", "<<", "<", "&&", "||", "|", "(", ")", NULL};
